...
''';
    ...
    test('command collector visitor', () {
      final configContent = '''
set \$mod Mod4
bindsym \$mod+Return exec terminal
...
''';
  ...
  group('Chained State System', () {
    test('should support variable scoping with context hierarchy', () {
      final processor = ConfigProcessor();
      processor.registerCommandHandler(SetCommandHandler());
      ...
      expect(blockContext.getVariable('global_var'), 'block_override'); // Shadowed
      expect(blockContext.getVariable('block_var'), 'block_value'); // Local
      expect(blockContext.getVariable('another_global'), 'another_value'); // Inherited
      
      // Test variable expansion from block context
      final expanded = blockContext.expandVariables('Block: \$global_var, \$block_var, \$another_global');
      ...
      expect(processor.context.getVariable('global_var'), 'global_value'); // Back to original
      expect(processor.context.getVariable('block_var'), null); // No longer accessible
      expect(processor.context.getVariable('another_global'), 'another_value'); // Still accessible
    });
    
    test('should handle nested block scoping', () {
      final processor = ConfigProcessor();
...
/// Test exec handler.
class _TestExecHandler implements CommandHandler {
  @override
